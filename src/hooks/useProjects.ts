...
function mapRow(row: DbRow): Project {
  return {
    ...
    projectCategory: row.project_category as Project['projectCategory'],
...
export function useProjects() {
  ...
  const addProject = useCallback(async (project: Project) => {
    ...
    const { error } = await supabase.from('projects_list').insert(row);
  ...
  const updateProject = useCallback(async (id: string, updates: Partial<Project>) => {
    ...
    const { error } = await supabase.from('projects_list').update(row).eq('id', id);
    if (!error) setProjects(prev => prev.map(p => p.id === id ? { ...p, ...updates } : p));
    return error;
  }, []);

  const deleteProject = useCallback(async (id: string) => {
    const { error } = await supabase.from('projects_list').delete().eq('id', id);
