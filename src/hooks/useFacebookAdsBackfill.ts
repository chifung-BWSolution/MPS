...
import { invokeFacebookAdsBackfill } from '@/lib/facebookAdsApi';
import type { FacebookAdsBackfillJob } from '@/types/facebookAds';

type JobDb = {
  id: string;
  status: FacebookAdsBackfillJob['status'];
  ...
  updated_at: string;
  meta: FacebookAdsBackfillJob['meta'] | null;
};

function mapJob(row: JobDb): FacebookAdsBackfillJob {
  return {
    id: row.id,
    status: row.status,
...
function mapJobFromApi(raw: Record<string, unknown>): FacebookAdsBackfillJob {
  return mapJob({
    id: String(raw.id),
    ...
    meta: (raw.meta as FacebookAdsBackfillJob['meta']) || null,
  });
}

export function useFacebookAdsBackfill() {
  const { session } = useAuth();
  const [job, setJob] = useState<FacebookAdsBackfillJob | null>(null);
  ...
  const start = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await invokeFacebookAdsBackfill('start');
  ...
  const pause = useCallback(async () => {
    ...
    setWorking(true);
    try {
      const res = await invokeFacebookAdsBackfill('pause', job.id);
      if (res.job) setJob(mapJobFromApi(res.job));
      return { ok: true as const };
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
  ...
  const resume = useCallback(async () => {
    ...
    setWorking(true);
    try {
      const res = await invokeFacebookAdsBackfill('resume', job.id);
      if (res.job) setJob(mapJobFromApi(res.job));
      setAutoRun(true);
      return { ok: true as const };
    } catch (e) {
  ...
  const cancel = useCallback(async () => {
    ...
    setWorking(true);
    try {
      const res = await invokeFacebookAdsBackfill('cancel', job.id);
      if (res.job) setJob(mapJobFromApi(res.job));
      return { ok: true as const };
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
  ...
  const stepOnce = useCallback(async () => {
    ...
    try {
      const res = await invokeFacebookAdsBackfill('step', current.id);
      if (res.job) {
