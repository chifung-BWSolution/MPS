...
export function useClientProjects() {
  ...
  const addProject = useCallback(async (project: Project) => {
    ...
    const { error } = await supabase.from('client_project').insert(row);
  ...
  const updateProject = useCallback(async (id: string, updates: Partial<Project>) => {
    ...
    const { error } = await supabase.from('client_project').update(row).eq('id', id);
    if (!error) setProjects(prev => prev.map(p => p.id === id ? { ...p, ...updates } : p));
    return error;
  }, []);

  const deleteProject = useCallback(async (id: string) => {
    const { error } = await supabase.from('client_project').delete().eq('id', id);
