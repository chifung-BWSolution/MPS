...
import { companies as mockCompanies, brands as mockBrands, projects, yearPlans } from '@/data/mockData';
...
export function BrandManagementSettings() {
  ...
  const [brandsData, setBrandsData] = useState<Brand[]>(mockBrands);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingBrand, setEditingBrand] = useState<Brand | null>(null);

  // Filter brands based on search and company
  ...
  const getCompanyName = (companyId: string) => {
    ...
    return company?.companyNameZh || '未知公司';
  };
  ...
  // Get active project count for a brand
  const getActiveProjectCount = (brandId: string) => {
    return projects.filter((p) => p.brandId === brandId && p.status === 'active').length;
  };

  // Get year plan progress for a brand (mock)
  ...
  const handleSave = (formData: Partial<Brand>) => {
    if (editingBrand) {
      setBrandsData((prev) =>
        prev.map((b) =>
          b.id === editingBrand.id ? { ...b, ...formData, updatedAt: new Date().toISOString() } : b
        )
      );
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {isModalOpen && (
        <BrandModal
          brand={editingBrand}
          companies={mockCompanies.filter(c => c.isActive)}
          existingCodes={brandsData.map((b) => b.brandCode)}
          onSave={handleSave}
            ...
            <div className="space-y-2.5 mb-4">
              <div className="flex items-center justify-between text-[12px]">
                ...
                <span className="font-medium">
                  <span className="text-[10px] font-mono bg-muted px-1.5 py-0.5 rounded mr-1.5">
                    {getCompanyCode(brand.companyId)}
                  </span>
              ...
              <div className="flex items-center justify-between text-[12px]">
                ...
                <div className="flex items-center gap-2">
                  <div
                    className="w-4 h-4 rounded-sm border border-[rgba(13,26,45,0.1)]"
                ...
                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={cn(
                      ...
                      yearPlan.progress >= 70 ? 'bg-teal-500' : yearPlan.progress >= 40 ? 'bg-amber-500' : 'bg-rose-400'
                    )}
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={cn(
                        ...
                        brand.isActive ? 'bg-teal-50 text-teal-700' : 'bg-amber-50 text-amber-700'
                      )}
                    >
                    ...
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          brand.isActive ? 'text-amber-600 hover:text-amber-700' : 'text-teal-600 hover:text-teal-700'
                        )}
                      >
                        {brand.isActive ? '停用' : '啟用'}
                      </button>
...
function BrandModal({
  ...
  const validate = () => {
    ...
    else if (!brand && existingCodes.includes(form.brandCode.trim().toUpperCase())) {
      errs.brandCode = '品牌編碼已存在';
    }
    if (!form.brandNameZh?.trim()) errs.brandNameZh = '必填';
    if (!form.brandNameEn?.trim()) errs.brandNameEn = '必填';
    setErrors(errs);
            ...
            <div>
              ...
              <select
                ...
                className={cn(
                  ...
                  brand && 'bg-muted cursor-not-allowed'
                )}
              >
                <option value="">— 請選擇公司 —</option>
                {companies.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.companyCode} - {c.companyNameZh}
                  </option>
                ))}
              </select>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FieldInput
                ...
                onChange={(v) => updateField('brandCode', v)}
                error={errors.brandCode}
                placeholder="如 BW, ACI"
                disabled={!!brand}
              />
              <FieldInput
                label="中文品牌名"
                value={form.brandNameZh || ''}
                onChange={(v) => updateField('brandNameZh', v)}
                error={errors.brandNameZh}
                placeholder="志豐企業"
              />
              <FieldInput
                label="英文品牌名"
                value={form.brandNameEn || ''}
                onChange={(v) => updateField('brandNameEn', v)}
                error={errors.brandNameEn}
                placeholder="BWDesign Centre"
