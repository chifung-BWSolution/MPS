...
function rebuildRows(
  ...
  const installments = planBulkExpenseInstallmentNumbers({
    projectRows,
    supplierTypesId: next.supplierTypesId,
...
export function emptyBulkExpenseDraft(
  projectRows: QuotationExpense[],
  signedDate?: string,
  handoverDate?: string,
  defaultTypeId = '',
...
export function bulkExpenseDraftFromRows(
  groupRows: QuotationExpense[],
  projectRows: QuotationExpense[],
  signedDate?: string,
  handoverDate?: string,
): BulkDraft {
...
export function PitchingBulkExpenseDialog({
  ...
  open: boolean;
  editingRows: QuotationExpense[] | null;
  projectRows: QuotationExpense[];
  signedDate?: string;
  handoverDate?: string;
  saving: boolean;
  ...
  const [draft, setDraft] = useState<BulkDraft>(() =>
    emptyBulkExpenseDraft(projectRows, signedDate, handoverDate, defaultTypeId),
  );
  ...
  useEffect(() => {
    ...
    setDraft(
      editingRows?.length
        ? bulkExpenseDraftFromRows(editingRows, projectRows, signedDate, handoverDate)
        : emptyBulkExpenseDraft(projectRows, signedDate, handoverDate, defaultTypeId),
    );
  ...
  const handleSave = async () => {
    const validationError = validateBulkExpenseInput(draft);
    if (validationError) {
      toast.error(validationError);
      return;
    }
    const numbers = draft.rows.map((row) => parseInstallmentNumber(row.installmentNumber));
    const collision = findExpenseInstallmentCollision(
      projectRows,
      draft.supplierTypesId,
      draft.supplierId,
      numbers,
    ...
    const items = draft.rows.map((row) => {
      ...
      const input: QuotationExpenseWriteInput = existing
        ? expenseToWriteInput(existing, {
            supplierTypesId: draft.supplierTypesId,
        ...
        <section className="space-y-4">
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      patchHeader(
                        {
                          supplierTypesId: type.id,
                          supplierId: draft.supplierTypesId === type.id ? draft.supplierId : '',
                        },
                        { resetInstallments: type.id !== draft.supplierTypesId },
                      )
                    }
                    className={cn(
          ...
          <div>
            ...
            <SearchableSelect
              ...
              onValueChange={(supplierId) =>
                patchHeader({ supplierId }, { resetInstallments: supplierId !== draft.supplierId })
              }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-1">填寫後平均分配至各期，可再調整各期應付</p>
            </div>
            <div>
              ...
              <Input
                type="number"
                min="1"
                max={MAX_BULK_EXPENSE_INSTALLMENT_COUNT}
                step="1"
          ...
          <div className="flex items-end justify-between gap-3 flex-wrap">
            ...
            <div className="text-right">
              <p className={cn('text-[12px]', billedMismatch ? 'text-rose-700' : 'text-muted-foreground')}>
                應付合計 {formatExpenseMoney(billedSum, draft.currency)}
                {draft.currency !== DEFAULT_CURRENCY ? `（${formatMoney(toHkd(billedSum, draft.currency))}）` : ''}
                {totalValue != null && draft.totalAmount.trim() ? ` ／ 總額 ${formatExpenseMoney(totalValue, draft.currency)}` : ''}
              </p>
              {billedMismatch && (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-muted/10">
                    ...
                    <th className="text-left text-[12px] font-medium text-muted-foreground px-3 py-2">應付金額 Billed *</th>
                  </tr>
                </thead>
                <tbody>
