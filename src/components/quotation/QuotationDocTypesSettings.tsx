...
import type { QuotationDocType } from '@/lib/quotationDocs';
...
export function QuotationDocTypesSettings() {
  const { types, loading, error, addType, updateType, deleteType, countUsage } = useQuotationDocTypes();
  const [search, setSearch] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<QuotationDocType | null>(null);
  const [form, setForm] = useState<TypeForm>(emptyForm());
  const [saving, setSaving] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<QuotationDocType | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return types.filter((type) => !q || type.display.toLowerCase().includes(q));
  }, [types, search]);
  ...
  const openEdit = (type: QuotationDocType) => {
    setEditing(type);
    setForm({
  ...
  const handleSave = async () => {
    ...
    toast.success(editing ? '已更新文件類型' : '已新增文件類型');
    setModalOpen(false);
    setEditing(null);
  };

  const handleToggleActive = async (type: QuotationDocType) => {
  ...
  const handleDeleteClick = async (type: QuotationDocType) => {
    ...
    if (usage.docCount > 0) {
      reasons.push(`仍有 ${usage.docCount} 份文件使用此類型，無法刪除`);
    }
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">
                  {search.trim() ? '沒有符合的文件類型' : '尚未建立文件類型。請按「新增類型」。'}
                </td>
              </tr>
            )}
            {!loading &&
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <Input
              value={form.display}
              onChange={(e) => setForm((prev) => ({ ...prev, display: e.target.value }))}
              placeholder="例如 報價單"
          ...
          <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2.5">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">
                {form.isActive ? '啟用中，可在項目文件選用' : '已停用，不會出現在新的選單'}
              </div>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              ...
              disabled={saving || !form.display.trim()}
