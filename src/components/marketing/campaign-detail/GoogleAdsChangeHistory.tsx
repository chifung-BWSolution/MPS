...
const FILTERS: { id: 'all' | GoogleAdsChangeHistoryCategory; label: string }[] = [
  { id: 'all', label: '所有變更' },
...
function toolLabel(clientType: string) {
  ...
  return CLIENT_LABELS[clientType] || clientType.replace(/_/g, ' ');
}
...
export function GoogleAdsChangeHistory({
  sessions,
  campaignName,
  loading,
  error,
  ...
  sessions: GoogleAdsChangeHistorySession[];
  campaignName: string;
  loading: boolean;
  error: string | null;
  ...
  return (
    <div className="space-y-3">
      <div className="text-[12px] text-muted-foreground">
        變更記錄來自 Google Ads 變更事件，每一列是一次操作。欄位明細只保留近 30 日。
        ...
        {!detailAvailable ? <span className="ml-1">所選區間沒有落在近 30 日內。</span> : null}
      </div>
