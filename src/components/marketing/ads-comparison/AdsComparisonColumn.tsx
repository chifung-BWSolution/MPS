...
import { ADS_COMPARE_METRICS, isAdsCompareMetric } from '@/lib/adsDailySeries';
import type { DateRangePreset } from '@/types/googleAds';
import type {
  AdsCompareCatalog,
  AdsCompareColumnFilters,
  AdsCompareMetric,
  AdsComparePlatform,
} from '@/types/adsComparison';
...
export function AdsComparisonColumn({
  ...
  filters: AdsCompareColumnFilters;
  onChange: (patch: Partial<AdsCompareColumnFilters>) => void;
  copySources: { index: number }[];
  onCopyFrom: (sourceIndex: number) => void;
}) {
  const { platform, campaignKey, metric, preset, customFrom, customTo } = filters;
  ...
  const [range, setRange] = useState(() =>
    resolveDateRange(preset, customFrom, customTo, dataMinDate, dataMaxDate),
  );
  ...
  const onPlatformChange = (next: AdsComparePlatform) => {
    onChange({
      platform: next,
      campaignKey: next === platform ? campaignKey : '',
    });
  };
    ...
    <section className="bg-white border border-[rgba(13,26,45,0.08)] rounded-md shadow-card p-3 space-y-3 min-w-0">
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <Select value={preset} onValueChange={(v) => onChange({ preset: v as DateRangePreset })}>
        ...
        <SearchableSelect
          value={campaignKey}
          onValueChange={(value) => onChange({ campaignKey: value })}
        ...
        <Select value={metric} onValueChange={(v) => onChange({ metric: v as AdsCompareMetric })}>
          <SelectTrigger className="w-full h-9 text-[13px] bg-white">
        ...
        <div className="pt-0.5">
          <div className="text-[10px] text-muted-foreground mb-1">複製其他欄位篩選</div>
          <div className="flex flex-wrap gap-1.5">
            {copySources.map((source) => (
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-8 px-2 text-[11px] bg-white"
                onClick={() => onCopyFrom(source.index)}
              >
                <Copy size={12} />
                從欄位 {source.index + 1} 複製
              </Button>
            ))}
          </div>
        </div>
          ...
          kpis.map((item) => {
            ...
            return (
              <AdsKpiCard
                ...
                onClick={metricId ? () => onChange({ metric: metricId }) : undefined}
              />
            );
          })
        )}
